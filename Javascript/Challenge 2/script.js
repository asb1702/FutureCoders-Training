...
function popup (exhibition) { 
	...
	for (var i = 0; i<=4; i++) { //Loop runs for each event
		...
		var exhibitionCode = gallerydata.exhibitions [i].exhibition.exhibitionCode; 
		var exhibitionName = gallerydata.exhibitions [i].exhibition.exhibitionName; 
		var exhibitionStart = gallerydata.exhibitions [i].exhibition.exhibitionStart; 
		exhibitionStart = new Date (exhibitionStart);
		var exhibitionEnd = gallerydata.exhibitions [i].exhibition.exhibitionEnd; 
		exhibitionEnd = new Date (exhibitionEnd);
		
		var artist = gallerydata.exhibitions [i].exhibition.artist; 
		if (timeWindowStart<exhibitionStart && exhibitionStart<timeWindowEnd) {
			//If the event has not expired
			var node = document.createElement("p");
			node.innerHTML = "\nExhibition Code: " + exhibitionCode.toString() + "\n  |  Exhibition Name: " + exhibitionName.toString() + "\n  |  Start date: " + exhibitionStart.toDateString() + "\n  |  End date: " + exhibitionEnd.toDateString() + "\n  |  Artist: " + artist.toString();
			...
			var node = document.createElement("p");
			node.innerHTML = "\nExhibition Code: " + exhibitionCode.toString() + "\n  |  Exhibition Name: " + exhibitionName.toString() +"\n  |  Exhibition expired by " + diff + " hours";
			document.getElementById("printHTML").appendChild(node);
		}
	}
	...
	document.getElementById("totalHours").innerHTML = "Total expiration time: " + totalDays + " days and " + totalHours + " hours";
}
